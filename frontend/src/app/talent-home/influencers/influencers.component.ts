...
export class InfluencersComponent implements OnInit {
  ...
  getCities(){
    this.service.getCities().subscribe((response: any) => {
  ...
  deleteInfluencer(inputdata: any) {
    alertify.confirm(
      'Remove User',
      'Do you want to remove this influencer ?',
      () => {
  ...
  onRowClicked(row: any) {
    this.route.navigate([`home/talent/influencerProfile/${row}`]);
  }
  ...
  applyFilter() {
    this.dataSource.filterPredicate = (
      ...
      return (
        ...
        (!this.filterCriteria.nationality ||
          data.Nationality.trim().toLowerCase() ===
            this.filterCriteria.nationality) &&

        (!this.filterCriteria.city ||
          (data.CityLocation && data.CityLocation.trim().toLowerCase() ===
          this.filterCriteria.city))
