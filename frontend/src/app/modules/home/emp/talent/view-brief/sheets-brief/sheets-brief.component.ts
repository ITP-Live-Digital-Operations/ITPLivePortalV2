...
export class SheetsBriefComponent {
  ...
  public handleFileInput(event: Event, n?: number): void {
    ...
    if (
      ...
      this.toastrService.error(
        'Invalid file or wrong file type. Please upload a valid file.'
      );
    }
  }
  ...
  private uploadFileXlsx(): void {
    if (this.fileToUpload) {
      this.isLoading = true; // Start the buffer animation
        ...
        .subscribe(
          (data) => {
            ...
            if (this.budgetSheetId != null) {
              this.fileService
                ...
                .subscribe(
                  (deleteData) => {
                    ...
                    this.isLoading = false; // End the buffer animation
                  },
                  (deleteError) => {
                    this.toastrService.error(
              ...
              this.childEvent.emit('event');
              this.isLoading = false; // End the buffer
            }
          },
          (uploadError) => {
  ...
  private uploadFilePPTX(): void {
    if (this.fileToUpload) {
            ...
            if (this.presentationId != null) {
              this.fileService
                ...
                .subscribe(
                  (deleteData) => {
                    this.toastrService.success(
                      'File uploaded and old presentation deleted successfully!'
                    );
                    this.childEvent.emit('event');
                    this.isLoading = false; // End the buffer
                  },
                  (deleteError) => {
                    this.toastrService.error(
                      'Error deleting old presentation!'
                    );
                    console.error(deleteError);
                    this.isLoading = false; // End the buffer
                  }
      ...
      this.toastrService.error(
        'Invalid file or wrong file type. Please upload a valid PPTX file.'
      );
    }
  }
      ...
      this.fileService
        ...
        .subscribe(
          (data) => {
            ...
            if (this.pdfId != null) {
              this.fileService.deletePDFFile(this.pdfId).subscribe(
                (deleteData) => {
                  this.toastrService.success(
                    'File uploaded and old PDF deleted successfully!'
                  );
  ...
  protected chooseFile(): void {
    const dialogRef = this.dialog?.open(MainTableComponent, {
      width: '90%',
      height: '90%',
